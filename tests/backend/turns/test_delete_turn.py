...
@pytest.fixture
def group_turn(client: ApiClient, open_barbershop_id: int) -> dict:
...
def test_status(client: ApiClient, solo_turn: dict) -> None:
    """
    Successful turn deletion returns 204.
    """

    response = client.turns.delete_turn(solo_turn["turn_id"])
    assert_status(response, HttpStatus.NO_CONTENT)


def test_turn_no_longer_accessible(client: ApiClient, solo_turn: dict) -> None:
    """
    Deleted turn returns 404 on GET.
    """

    turn_id = solo_turn["turn_id"]
    client.turns.delete_turn(turn_id)
    response = client.turns.get_turn(turn_id)
...
def test_group_leader_deletion_cancels_entire_group(
    client: ApiClient, group_turn: dict
) -> None:
    """
...
def test_group_leader_deletion_restores_leader_status(
    client: ApiClient, open_barbershop_id: int, group_turn: dict
) -> None:
    """
...
def test_member_turn_deletion_does_not_cancel_leader(
    ...
    first_member_turn_id = group_turn["member_turn_ids"][0]
    client.turns.delete_turn(first_member_turn_id)
