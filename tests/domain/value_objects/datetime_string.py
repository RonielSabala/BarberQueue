...
_FIXED_DATETIME_LENGTH = 19
_DATETIME_PATTERN = re.compile(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$")


@dataclass(slots=True, frozen=True)
class DateTimeString(StringField):
    _min_len = _FIXED_DATETIME_LENGTH
    _max_len = _FIXED_DATETIME_LENGTH
    _pattern = _DATETIME_PATTERN
    _pattern_error_msg = "must be a valid datetime in format YYYY-MM-DD HH:MM:SS"
